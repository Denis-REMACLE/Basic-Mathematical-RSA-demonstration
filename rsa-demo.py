...
def factorise_p_and_q(p, q):
    print(
        "Secondly we must factorise P and Q to get the modulus (N) for both private and public key.\n"
    )
    print(
        "The modulus is a part of the public key : this is why RSA is threatened by the Shor's algorithm."
    )
    print(
        "The shors algorithm finds the primes factors of an integer using quantum computers\n"
    )
    n = p * q
...
def get_phi_n(p, q):
    print(
        "Thirdly we must get the λ(n) (the Carmichael's totien function) or the φ(n) (the Euler's totien function).\n"
    )
    phi = (p - 1) * (q - 1)
...
def find_relative_primes(phi):
    print(
        "Fourthly we must chose a number e thats is relatively prime with λ(n) or φ(n) depending on which one you calculated\n"
    )
...
def cipher(msg, public_key):
    ...
    for letter in msg:
        ...
        print(
            f"Ciphering {letter} (ASCII value = {ord(letter)}) : pow({ord(letter)}, {public_key['E']})%{public_key['N']} = {value}"
        )
...
def decipher(msg, private_key):
    ...
    for letter in msg:
        ...
        print(
            f"Deciphering {letter}\t:\tpow({letter}, {private_key['SecretExponent']})%\t{private_key['N']}\t=\t{ord(value)}\t=\t{value}"
        )
...
def main():
    ...
    parser.add_argument(
        "-r", "--random", help="Randomize P and Q", action="store_true", default=False
    )
    parser.add_argument(
        "-m", "--msg", help="String to use for the demo", default="hello", type=str
    )
    parser.add_argument(
        "-d",
        "--decipher",
        help="String to decipher using the generated key",
        nargs="+",
        type=int,
    )
    parser.add_argument(
        "-c",
        "--cipher",
        help="String to cipher using the generated key",
        default="",
        type=str,
    )
    parser.add_argument(
        "-s", "--shor", help="Use shor's algorithm to break the key", type=int
    )
    parser.add_argument(
        "-b", "--break_key", help="Use bruteforce to break the key", type=int
    )
    ...
    print(
        f"The private key is composed of the factorisation of the primes number and the secret exponent.\n{private_key}\n"
    )
    print(
        f"The public key is composed of the factorisation of the primes number and the modular inverse.\n{public_key}\n"
    )

    print(
        f"Let's imagine Bob wants to say hello to Alice, he'll use Alice's public key to cipher the letter array  \"{args.msg}\"."
    )
